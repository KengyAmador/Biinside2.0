                ...
                action: function () {
                   ...
                   $('#frmClientes').bootstrapValidator('resetForm',true);
						...
						$('#exampleModal').on('show.bs.modal', function (event) {
						  ...
						  var modal = $(this);	
						  modal.find('.modal-title').text('Nuevo rango');
						  $("#rango").val('');
						  $("#precio").val('');
					});
                ...
                action: function () {
                    ...
                    if(table.rows( { selected: true } ).count() == 1)
                	{
						...
						$('#exampleModal').on('show.bs.modal', function (event) {
							  ...
							  $("#rango").val(table.rows( { selected: true } ).data()[0].rango);
						  	  $("#precio").val(table.rows( { selected: true } ).data()[0].express);
						});
						$('#exampleModal').modal('show');
                	}
...
function abrirEliminar(){
	$('#borrarModal').on('show.bs.modal', function (event) {
	  var button = $(event.relatedTarget);
	  var modal = $(this);
	  modal.find('.modal-title').text('Eliminar el rango: ' + codigoSeleccionado.codigo);
	});
}
...
function procesarCliente()
{
    ...
    {
		//Edita
		var rango = $("#rango").val().toUpperCase();
