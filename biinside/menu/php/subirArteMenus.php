...
if(isset($_POST["codMenuSubirProducto"])){ //Validar el arte
    $codigo = $_POST["codMenuSubirProducto"];
}

$imagename= $_FILES["produtoArte"]["name"]; //Tomar la imagen
$folder="/sistema/menu/artes/"; //Carpeta donde guardar
$urlImagen = $_SERVER['DOCUMENT_ROOT'].$folder.$valuerandomString.$_FILES["produtoArte"]["name"]; //Url de imagen a guardar
$urlFinal = "http://3.16.186.99".$folder.$valuerandomString.$_FILES["produtoArte"]["name"];



if(move_uploaded_file($_FILES["produtoArte"]["tmp_name"], $_SERVER['DOCUMENT_ROOT'].$folder.$valuerandomString.$_FILES["produtoArte"]["name"])){//Si se pudo subir el archivo
