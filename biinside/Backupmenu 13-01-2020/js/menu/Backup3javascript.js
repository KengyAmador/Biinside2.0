                ...
                action: function () {
                   ...
                   $('#frmCategoria').bootstrapValidator('resetForm',true);
						...
						$('#modal_menuCategoria').on('show.bs.modal', function (event) {
						  ...
						  var modal = $(this);	
						  modal.find('.modal-title').text('Registro e eliminación de accesos');
                ...
                action: function () {
                   ...
                   $('#frmDetalleMenu').bootstrapValidator('resetForm',true);
						...
						$('#modal_detallemenu').on('shown.bs.modal', function (event) {
						  var button = $(event.relatedTarget);
						  var modal = $(this);	
						  modal.find('.modal-title').text('Nuevo Menú');
						  $('#menuSelect').val('');
						  $('#accionSelect').val('');
						  $('#valorDetMenu').val('');
						  $('#precioDetMenu').val('');
                ...
                action: function () {
                    ...
                    if(table.rows( { selected: true } ).count() == 1)
                	{
						...
						$('#modal_menuExtras').on('show.bs.modal', function (event) {
							  var button = $(event.relatedTarget);
							  var modal = $(this);	
							  modal.find('.modal-title').text('Extras Menú: ');
							  $('#precioextra').val('');
							 //Oculto las alertas
	...
	$.ajax({
		...
		success:function(data)
		{
			...
			data.map((item)=>{
				...
				'<td>'+item.nombre+'</td>'+
				'<td>'+item.descripcion+'</td>'+
				'<td><img src="'+item.imagen+'" width="30"></img></td>'+
				...
				'<td>'+
					'<div class="dropdown">'+
						...
						'<div class="dropdown-menu text-center " aria-labelledby="dropdownMenu2">'+
							...
							'<button class="btn btn-success btn-sm ml-1" title="Subir arte" ><i class="fa fa-user" aria-hidden="true"></i>📤</button>'+
	...
	$.ajax({
		...
		success:function(data)
		{
			...
			data.map((item)=>{
				...
				'<td><button class="btn btn-danger btn-sm" onclick="eliminarMenusMensajeAlerta('+item.codigo+')"><i class="fa fa-address-book" aria-hidden="true"></i>Eliminar</button></td>'+
			  	'</tr>';
			})
