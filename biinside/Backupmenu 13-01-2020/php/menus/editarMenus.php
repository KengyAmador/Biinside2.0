...
<?php
    ...
    try {
      ...
      $consulta = $conexion->prepare('CALL `sp_EditarMenus`(?,?,?,?,?)'); //Hacer la consulta
      $consulta->bindValue(1, $codigo, PDO::PARAM_INT);//Parametros
      $consulta->bindValue(2, $categoria, PDO::PARAM_INT);//Parametros
	  $consulta->bindValue(3, $nombre, PDO::PARAM_STR);//Parametros
	  $consulta->bindValue(4, $descripcion, PDO::PARAM_STR);//Parametros
