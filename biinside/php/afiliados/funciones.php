...
<?php
    ...
    if(isset($_POST["puntos"])){ //Tipo de asignacion de puntos
        $puntos = $_POST["puntos"];
    }
    else $puntos = "";
	...
	if( $_GET['peticion'] == 'guardar' ) //Peticion relacionada a los productos
	{
		...
		{
			if($cedula !== "" && $nombre !== "" && $encargado !== "" && $provincia !== "" && $canton !== "" && $distrito !== "" && $correo !== "" && $telefono !== "" && $direccion !== "" && $industria !== "" && $puntos !== "")
			{
				guardarAfiliado($cedula, $nombre, $encargado, $provincia, $canton, $distrito, $correo, $telefono, $direccion, $industria, $puntos);
			}
			else
			{
				echo 2;
	...
	function guardarAfiliado($pCedula, $pNombre, $pEncargado, $pProvincia, $pCanton, $pDistrito, $pCorreo, $pTelefono, $pDireccion, $pIndustria, $pPuntos)
	{
		try{
		    ...
		    $consulta->bindValue(11, $pPuntos, PDO::PARAM_STR);//Parametros
